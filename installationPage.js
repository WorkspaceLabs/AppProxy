...
const cacheName = 'AppProxy';
const workerUrl = '/Apps/AppProxy/install';
const configPath = '/Apps/AppProxy/config';
const templateUrl = 'https://workspacelabs.github.io/AppProxy/rules/index.json';
const templatesPromise = fetch(templateUrl).then(response => response.json());
const pageTemplate = () => p `
<label for="templateSelect"></label>
<select id="templateSelect" disabled>
    <option>Loading Template...</option>
</select>
<button id="installButton" hidden>Install</button>
<button id="uninstallButton" hidden>Uninstall</button>
<br />
<pre id="statusLabel">Ready</pre>
`;
let templates;
let selectedTemplateName;
let selectedRules;
function renderPage() {
    w(pageTemplate(), document.body);
    installButton.addEventListener('click', onInstall);
    uninstallButton.addEventListener('click', onUninstall);
}
async function onLoad() {
    ...
    option.value = '';
    option.text = '[ Please select template ]';
    templateSelect.appendChild(option);
    templates.forEach(x => {
        const option = document.createElement('option');
        option.value = option.text = x.name;
        templateSelect.appendChild(option);
    });
...
async function onTemplateChange() {
    ...
    installButton.hidden = uninstallButton.hidden = true;
    selectedTemplateName = templateSelect.value;
    const selectedTemplate = templates.find(x => x.name === selectedTemplateName);
    if (selectedTemplate) {
        const url = new URL(selectedTemplate.path, templateUrl);
        selectedRules = await fetch(url.href).then(response => response.json());
        const hasActiveWorker = Boolean(await getActiveWorker(selectedRules.scope));
        installButton.hidden = hasActiveWorker;
        uninstallButton.hidden = !hasActiveWorker;
    }
    templateSelect.disabled = false;
}
async function onUninstall(e) {
    const scope = selectedRules.scope;
    if (e !== null) {
        uninstallButton.disabled = true;
        statusLabel.textContent = '';
    }
    try {
        const activeWorker = await getActiveWorker(scope);
        if (activeWorker !== undefined) {
            const result = await activeWorker.unregister();
            if (result) {
                statusLabel.textContent = 'Uninstalled';
            }
            else {
                throw 'Error while unregister active worker';
            }
        }
        const cache = await caches.open(cacheName);
        await cache.delete(new Request(configPath + scope));
        if (e !== null) {
        ...
        let errorMessage = 'Unable to uninstall previous worker';
        if (err instanceof Error) {
            errorMessage += ' because ' + err.message;
        }
...
async function onInstall(e) {
    let registration;
    const scope = selectedRules.scope;
    if (!await onUninstall(null))
        return;
    try {
        const cache = await caches.open(cacheName);
        await cache.put(new Request(configPath + scope), createResponse(selectedRules));
        ...
        if (registration !== undefined) {
            ...
            throw 'Unable to install';
        }
    }
    catch (err) {
        let errorMessage = 'Unable to install';
        if (err instanceof Error) {
            errorMessage += ' because ' + err.message;
        }
        statusLabel.textContent = errorMessage;
...
async function getActiveWorker(scope) {
    const workers = await navigator.serviceWorker.getRegistrations();
    return workers.find(x => {
        if (x.active === null)
            return false;
        if (!x.scope.includes(scope))
            return false;
        return true;
    });
}
